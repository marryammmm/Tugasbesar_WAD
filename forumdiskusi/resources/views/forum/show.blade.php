...
@section('content')
    <div class="container">
        ...
        <div class="mb-5">
            ...
            <small class="text-muted">Dibuat oleh: <strong>{{ $forum->pengguna->nama }}</strong> | {{ $forum->created_at->format('d M Y') }}</small>
        </div>
            ...
            @forelse ($forum->posts as $post)
                <div class="list-group-item border rounded mb-3">
                    ...
                    <div class="d-flex justify-content-between">
                        <h5 class="mb-1 text-success"><i class="fas fa-user-circle"></i> {{ $post->user->nama }}</h5>
                    ...
                    @if (session('user_id') && session('user_id') == $post->pengguna_id)
                    ...
                    @if ($post->comments->count() > 0)
                        ...
                        <div class="comments-list">
                            @foreach ($post->comments as $comment)
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <h5 class="card-title">{{ $comment->user->nama }}</h5>
                                        ...
                                        @if (session('user_id') && session('user_id') == $comment->pengguna_id)
                                            <div class="d-flex justify-content-end">
